...
HOST = "dev.verta.ai"
PROJECT_NAME <-  "MTCARS Regression"
...
## Initialize Client, set up some experiments
...
```{r create_lm_model}
run <- set_experiment_run(name="MTCars Experiment 1",desc = "MtCars Experiment")
log_run_hyperparameters(run,hyperparams = list(hyperparam1=1,hyperparam2=2))
log_run_hyperparameters(run,hyperparams = list(formula="mpg~."))
...
```{r create_lm_model_2}
run2 <- set_experiment_run(name="MTCars Experiment 2",
                           desc = "MTCars Experiment 2")
...
run_log_metric(run2,key = "rsquared",value = rsq_2)
...
first_model_object <- list(
  ...
  formula_obj = "mpg~.",original_df = trainSet,required_packages=""
)

saveRDS(first_model_object,"mpg_experiment_object.RData")
run$log_model(model = "mpg_experiment_object.RData")
```
...
run$download_model("downloaded_model")
downloaded_model <- readRDS("downloaded_model/tmp9_h_qyst")# this 
...
registered_model_best_mtcars <- verta_get_or_create_registered_model(name="mtcars_best",
                                                      workspace = WORKSPACE)
